...
def redraw(): # Clean up the screen and start a new grid and new frame of pendulum with new coordinates
    ...
    text1 = font.render(f'Fitness error: {round(numpy.min(fitness_id),5)}', True, pygame.Color("white"), pygame.Color("black"))
    text2 = font.render(f'Fitness Code: {fitness_value[numpy.argmin(fitness_id)]}', True, pygame.Color("white"), pygame.Color("black"))
    text3 = font.render(f'Generation: {len(fitness_id)}', True, pygame.Color("white"), pygame.Color("black"))

    background.blit(text, (10,10))
    background.blit(text1, (10,40))
    background.blit(text2, (10,70))
    background.blit(text3, (150,10))
...
while not Out:
    ...
    if abs(Aacc)<0.000001 or angle*180/math.pi > 100 or angle*180/math.pi < -100:
        fitness_id.append(abs(angle*180/math.pi-Want_Result))
        fitness_value.append(random_a)
        print('=========')
        print("angle", angle*180/math.pi)
        print(random_a)
        print('=fitness=') #### vel
        print(fitness_value[numpy.argmin(fitness_id)])
        if numpy.min(fitness_id) <= 5.:
            print("RE")
            random_a = fitness_value[numpy.argmin(fitness_id)] + random.uniform(-1,1)/10000
        else:
            random_a = fitness_value[numpy.argmin(fitness_id)] + random.uniform(-1,1)/1000
    ...
    redraw()
# writedata.py
f = open("/home/kar7mp5/바탕화면/Coding/Center_of_gravity/AI.txt", 'w')
data = f"{fitness_value[numpy.argmin(fitness_id)]}"
f.write(data)
f.close()
pygame.quit()
